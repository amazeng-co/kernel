...
#define VERT_CONVERT    0   /* define if vertical screen with horizontal coords */
			 ...
			 u8 *key_value)
{
	...
	*key_value = point_data[3 + 8 * touch_num];
	...
	for (i = 0; i < touch_num; i++) {
        ...
        /* reverted mount situation */
        points[i].x = ts->pdata->abs_size_x - points[i].x;
        points[i].y = ts->pdata->abs_size_y - points[i].y;
        #endif

        dev_dbg(&ts->client->dev, "2-[%d][%d %d %d]\n",
...
int gtp_parse_dt_cfg(struct device *dev, u8 *cfg, int *cfg_len, u8 sid)
{
    ...
    sid = 0; /* liaokai@ default cfg-group0 */
